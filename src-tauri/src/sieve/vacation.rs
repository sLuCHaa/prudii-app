...
//!
//! Not a full Sieve parser: it understands comments, strings, `require`,
//! `vacation` and `if` with `currentdate` tests — everything else passes
//! through as opaque bytes, which is what keeps foreign rules intact.
